...
public class OrderBusinessArrayList implements Iterable<Action> {
  ...
  public void sorted() {
    synchronized (OrderBusinessArrayList.class) {
      ...
      for (Action action : actions) {
        ...
        voteMap.computeIfPresent(RouterSaver.class, (aClass, integer) -> integer + 2);
        voteMap.computeIfPresent(RootUser.class, (aClass, integer) -> integer + 1);
        // 如果没有dependList可能排序会出错，以上是增加一种默认情况
