...
public class RouteScanner {
     ...
     */
    public static void showRequestValue(List<String> filenames) throws ClassNotFoundException {
        //得到字节码文件  【只需要更改controller类名】
        log.info("当前方法为："+Thread.currentThread().getStackTrace()[1].getMethodName()+
        ...
        for (int i = 0; i < filenames.size(); i++) {
            ...
            if (n == 0) {
                log.info("当前方法为："+Thread.currentThread().getStackTrace()[1].getMethodName()+ " 当前类不存在requestMapping注解");
            } else {
                log.info("当前方法为："+Thread.currentThread().getStackTrace()[1].getMethodName()+ "  总共有" + values.size() + "个requestMapping注解");
                log.info("当前方法为："+Thread.currentThread().getStackTrace()[1].getMethodName()+ "  其值分别为：  ");
                for (int j = 0; j < values.size(); j++) {
                    log.info("当前方法为："+Thread.currentThread().getStackTrace()[1].getMethodName()+ values.get(j) + ", ");
                }
            }
        }
    }
    
    public static Set<String> getAllUrl(List<String> filenames) throws ClassNotFoundException {
        Set<String> urlSet = new HashSet<>();
        log.info("当前方法为："+Thread.currentThread().getStackTrace()[1].getMethodName()+"  共有  "
